...
RSpec.describe TrialUserAttribute, type: :model do
  describe 'validations' do
    context 'without key' do
      it 'is valid' do
        attribute = FactoryBot.create(:trial_user_attribute, key: nil)
        expect(attribute).to be_valid
      end
    end
    context 'without value' do
      it 'is valid' do
        attribute = FactoryBot.create(:trial_user_attribute, value_json: nil)
        expect(attribute).to be_valid
      end
    end
    context 'without study_id' do
      it 'is invalid' do
        expect { FactoryBot.create(:trial_user_attribute, trial_id: nil) }.to raise_error ActiveRecord::RecordInvalid
      end
    end
  end
  ...
  describe 'member functions' do
    ...
    let!(:attribute1) { FactoryBot.create :trial_user_attribute, trial_id: 1 }
    let!(:attribute2) { FactoryBot.create :trial_user_attribute, trial_id: 1 }
    let!(:attribute3) { FactoryBot.create :trial_user_attribute, trial_id: 2 }
    ...
    context 'when deleting using destroy_where' do
      it 'should delete any attribute with said study_id' do
        ...
        TrialUserAttribute.destroy_where attribute1.trial_id
